...
import '../premium/premium_checker.dart'; // Make sure to import this
...
class _DietPlanResultState extends State<DietPlanResult> {
  Map<String, Set<String>> loggedFoods = {};
  ...
  Widget build(BuildContext context) {
    ...
    final mealsPerDay = int.tryParse(widget.data['mealsPerDay']?.toString() ?? '3') ?? 3;
    final selectedStapleFoods = (widget.data['selectedStapleFoods'] as List<dynamic>?)?.cast<String>() ?? [];

    final double tdee = (widget.data['tdee'] is num)
    ...
    final Map<String, double> calorieDistribution = mealsPerDay == 4
        ? {'Breakfast': 0.20, 'Lunch': 0.30, 'Dinner': 0.30, 'Snacks': 0.20}
        : {'Breakfast': 0.25, 'Lunch': 0.35, 'Dinner': 0.40};

    final Map<String, double> caloriesPerMeal = {
      for (final meal in mealLabels)
        meal: calorieTarget * (calorieDistribution[meal] ?? 0)
    };
          ...
          return Padding(
            ...
            child: ListView(
              children: [
                ...
                // Premium: Track Your Calories Button
                PremiumChecker(
                  premiumChild: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => CalorieTrackerPage(calorieGoal: calorieTarget),
                        ),
                      );
                    },
                  ...
                  nonPremiumChild: ElevatedButton.icon(
                    onPressed: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text("Upgrade to Premium to use this feature!")),
                      );
                    },
                ...
                ElevatedButton(
                  onPressed: () => Get.toNamed('/diet_questions'),
                  child: const Text("Edit Diet Preferences"),
                ),

                const SizedBox(height: 30),
                _macroSummaryCard(calorieTarget, macros),
                ...
                const SizedBox(height: 10),
                ...mealLabels.map((mealName) => _buildMealCard(mealPlan, mealName)).toList(),
              ],
            ),
          );
        },
  ...
  Widget _buildMealCard(Map<String, List<FoodProduct>> mealPlan, String mealName) {
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            IconButton(
              ...
              onPressed: () async {
                final updatedMeal = await Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => EditMealPage(mealName: mealName, currentFoods: products),
                  ),
                );
                if (updatedMeal != null) {
                  setState(() => mealPlan[mealName] = updatedMeal);
                  final user = FirebaseAuth.instance.currentUser;
                  if (user != null) {
                    final updatedPlan = {
                      for (final entry in mealPlan.entries)
                        entry.key: entry.value.map((food) => food.toJson()).toList(),
                    };
                    await FirebaseFirestore.instance
                        .collection('users')
                        .doc(user.uid)
                        .update({'generatedDietPlan': updatedPlan});
                    ScaffoldMessenger.of(context).showSnackBar(
          ...
          ...products.map((product) {
            final alreadyEaten = loggedFoods[mealName]?.contains(product.name) ?? false;

            return PremiumChecker(
              premiumChild: _buildProductTile(product, alreadyEaten, mealName),
              nonPremiumChild: ListTile(
  ...
  Widget _buildProductTile(FoodProduct product, bool alreadyEaten, String mealName) {
    return ListTile(
      title: Text(product.name),
      subtitle: Text('${product.calories.toStringAsFixed(0)} kcal | '
          '${product.protein.toStringAsFixed(1)}g protein | '
          ...
          '${product.carbs.toStringAsFixed(1)}g carbs'),
      trailing: IconButton(
        icon: Icon(alreadyEaten ? Icons.check : Icons.check_circle,
            color: alreadyEaten ? Colors.green : Colors.grey),
        tooltip: alreadyEaten ? 'Remove from Log' : 'Mark as Eaten',
        onPressed: () async {
          setState(() {
            if (alreadyEaten) {
              loggedFoods[mealName]?.remove(product.name);
            } else {
              loggedFoods.putIfAbsent(mealName, () => {}).add(product.name);
            }
          });
          try {
            if (alreadyEaten) {
              await CalorieLogService.removeFoodFromToday(mealName, product);
            } else {
              await CalorieLogService.logMealForToday(mealName, [product]);
            }
          } catch (e) {
            setState(() {
              if (alreadyEaten) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('Failed to ${alreadyEaten ? 'remove' : 'log'} ${product.name}: $e')),
            );
          }
        },
      ),
  ...
  Widget _macroSummaryCard(double calorieTarget, Map<String, double> macros) {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Carbs: ${macros['carbs_g']?.toStringAsFixed(1)} g'),
        ]),
      ),
    );
  }
  ...
  Widget _macroTag(String emoji, String text) {
    return Chip(label: Text("$emoji $text"), backgroundColor: Colors.deepPurple.shade50);
  }
}
